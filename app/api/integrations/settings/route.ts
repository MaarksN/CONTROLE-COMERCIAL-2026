...
export async function GET() {
  ...
  try {
    ...
    return Response.json({
      ...
      googleOAuthConfigured: Boolean(settings.googleClientId && settings.googleClientSecret),
      googleClientIdMasked: maskSecret(settings.googleClientId),
      googleClientSecretMasked: maskSecret(settings.googleClientSecret),
...
export async function POST(request: Request) {
  ...
  try {
    ...
    await upsertIntegrationSettings(env.DB, {
      ...
      googleClientId: toUpdate(payload.googleClientId),
      googleClientSecret: toUpdate(payload.googleClientSecret),
      aiProvider,
    ...
    await writeAudit({
      ...
      detail: {
        ...
        googleClientId: toUpdate(payload.googleClientId) ? "updated" : undefined,
        googleClientSecret: toUpdate(payload.googleClientSecret) ? "updated" : undefined,
        aiProvider,
