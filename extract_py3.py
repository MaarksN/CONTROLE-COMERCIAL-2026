...
# Remove the first line of state_body because it's the component's signature
# The signature spans lines. We need to skip until the `{`
body_start = 0
brace_count = 0
found_brace = False
for i, char in enumerate("".join(lines[start_idx:])):
    if char == '{':
        found_brace = True
        body_start = i + 1
        break

state_body = "".join(lines[start_idx:])[body_start : "".join(lines[start_idx:]).find("  return (")]
